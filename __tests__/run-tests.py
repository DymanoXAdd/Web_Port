...
Run from the repo root:
    python "02 Build/__tests__/run-tests.py"
"""
...
def expect(actual):
    class Matcher:
        ...
        def to_contain(self, substring):
            assert substring in str(actual), f'Expected string to contain {repr(substring)}'
        ...
        class _Not:
            ...
            def to_contain(self, substring):
                assert substring not in str(self.val), \
                    f'Expected string NOT to contain {repr(substring)}'
            def to_be(self, expected):
...
# ──────────────────────────────────────────────────────────────────────────────
# Functions under test (Python re-implementations matching the TS originals)
# ──────────────────────────────────────────────────────────────────────────────
...
# ──────────────────────────────────────────────────────────────────────────────
# Read source files for source-code verification
# ──────────────────────────────────────────────────────────────────────────────

# This file is at: 02 Build/__tests__/run-tests.py
# Source files are at: 02 Build/
...
def _xss_img():
    ...
    e = escape_html(xss)
    expect(e).not_.to_contain("<")
    expect(e).not_.to_contain(">")
    expect(e).not_.to_contain('"')
    expect(e).to_contain("&lt;img")
test("neutralises img onerror XSS payload", _xss_img)

def _xss_script():
    xss = "<script>alert('xss')</script>"
    e = escape_html(xss)
    expect(e).not_.to_contain("<script>")
    expect(e).to_contain("&lt;script&gt;")
test("neutralises script tag XSS payload", _xss_script)
...
def _xss_event():
    ...
    expect(e).not_.to_contain('"')
    expect(e).to_contain("&quot;")
test("neutralises event-handler injection", _xss_event)

test("handles unicode without corruption", lambda:
    expect(escape_html("H\xe9llo caf\xe9")).to_be("H\xe9llo caf\xe9"))

test("escapeHtml called on name in email.ts", lambda:
    expect(email_src).to_contain("escapeHtml(name)"))
test("escapeHtml called on email in email.ts", lambda:
    expect(email_src).to_contain("escapeHtml(email)"))
test("escapeHtml called on subject in email.ts", lambda:
    expect(email_src).to_contain("escapeHtml(subject)"))
test("escapeHtml called on message in email.ts", lambda:
...
test("accepts name at minimum (2 chars)", lambda:
    expect(validate_contact_form({**valid, "name": "Jo"})["success"]).to_be(True))
test("rejects malformed email — no @", lambda:
...
test("accepts message at exact max (5000 chars)", lambda:
...
test("allows first request", _rl_first)

def _rl_n():
    store = make_store()
    for _ in range(RATE_LIMIT):
        assert check_rate_limit(store, "1.2.3.4", NOW) == True
test(f"allows exactly {RATE_LIMIT} requests per window", _rl_n)
...
test(f"blocks the {RATE_LIMIT + 1}th request", _rl_block)
...
test("different IPs tracked independently", _rl_independent)
...
def _rl_reset():
    ...
    for _ in range(RATE_LIMIT):
        check_rate_limit(store, "1.2.3.4", NOW)
    assert check_rate_limit(store, "1.2.3.4", NOW) == False
    assert check_rate_limit(store, "1.2.3.4", NOW + RATE_LIMIT_WINDOW + 1) == True
test("counter resets after window expires", _rl_reset)

# ── Suite 4: IP Extraction ────────────────────────────────────────────────────

describe("IP Extraction — getClientIp() anti-spoofing")

test("reads LAST x-forwarded-for entry", lambda:
    expect(get_client_ip({"x-forwarded-for": "1.2.3.4, 5.6.7.8, 99.99.99.99"})).to_be("99.99.99.99"))
test("handles single-entry forwarded header", lambda:
    expect(get_client_ip({"x-forwarded-for": "10.0.0.1"})).to_be("10.0.0.1"))
test("trims spaces around entries", lambda:
    expect(get_client_ip({"x-forwarded-for": "  1.2.3.4  ,  5.6.7.8  "})).to_be("5.6.7.8"))
test("falls back to x-real-ip", lambda:
    expect(get_client_ip({"x-forwarded-for": None, "x-real-ip": "10.20.30.40"})).to_be("10.20.30.40"))
test("returns 'unknown' when no headers", lambda:
    expect(get_client_ip({"x-forwarded-for": None, "x-real-ip": None})).to_be("unknown"))

def _no_first_entry():
    ip = get_client_ip({"x-forwarded-for": "SPOOFED, 5.6.7.8, real.edge.ip"})
    assert ip != "SPOOFED", f"Must not use first (spoofable) entry, got {ip}"
    assert ip == "real.edge.ip", f"Expected real.edge.ip, got {ip}"
test("does NOT use the first (client-spoofable) entry", _no_first_entry)

test("route source uses parts[parts.length-1]", lambda:
...
test("allows /",                    lambda: expect(is_allowed_path("/")).to_be(True))
...
describe("Source Code Verification — fixes confirmed in files")

test("CSP header configured in next.config.js",         lambda: expect(next_config_src).to_contain("Content-Security-Policy"))
test("HSTS header configured in next.config.js",        lambda: expect(next_config_src).to_contain("Strict-Transport-Security"))
test("CSP has frame-ancestors 'none'",                  lambda: expect(next_config_src).to_contain("frame-ancestors 'none'"))
test("CSP has form-action 'self'",                      lambda: expect(next_config_src).to_contain("form-action 'self'"))
test("CSP allows Sanity CDN",                           lambda: expect(next_config_src).to_contain("cdn.sanity.io"))
test("X-Content-Type-Options present",                  lambda: expect(next_config_src).to_contain("X-Content-Type-Options"))
test("X-Frame-Options present",                         lambda: expect(next_config_src).to_contain("X-Frame-Options"))
test("Referrer-Policy present",                         lambda: expect(next_config_src).to_contain("Referrer-Policy"))
test("Permissions-Policy present",                      lambda: expect(next_config_src).to_contain("Permissions-Policy"))
test("email route has generic 500 message",             lambda: expect(email_route_src).to_contain("An error occurred. Please try again later."))
test("old leaky error message removed",                 lambda: expect(email_route_src).not_.to_contain("Failed to send email"))
test("email route guards Content-Type",                 lambda: expect(email_route_src).to_contain("application/json"))
test("email route uses last forwarded-for entry",       lambda: expect(email_route_src).to_contain("parts[parts.length - 1]"))
test("revalidate route rejects GET with 405",           lambda: expect(revalidate_src).to_contain("405"))
test("revalidate route has ALLOWED_PATHS allowlist",    lambda: expect(revalidate_src).to_contain("ALLOWED_PATHS"))
test("revalidate route returns 401 on bad secret",      lambda: expect(revalidate_src).to_contain("401"))
...
print(f"  Failed:  {failed} {chr(0x274c) if failed > 0 else chr(0x2705)}")
...
if failed > 0:
    ...
    print("  Site is hardened and ready for deployment.\n")
